...
public class Server {
    ...
    public Server(int serverPort) {
        ...
        try {
            ...
            log.error("Unable to determine local host IP", e);
        }
        runSystem();
    }
    ...
    public void shutdown() {
        log.info("There is work in progress: " + workManager.isWorkInProgress());
        if (workManager.isWorkInProgress() || workManager.isWorkAvailable()){
            log.warn("Unable to shutdown due to work in progress or backlog!");
        } else {
    ...
    private Node createNewNode(InetAddress nodeIP, int nodePort, int inputMaxJobs) {
        ...
        log.debug(String.format("Created node object id=%d ip=%s port=%d maxJobs=%d",
    ...
    public void createNewWork(int duration) {
        ...
        log.debug(String.format("New work created id=%d duration=%d", workID, duration));
        Node tempNode = assignWorkCreated();
        if (tempNode != null){
            log.info(String.format("Work %d assigned to node %d", newWork.getWorkID(), tempNode.getNodeID()));
            newWork.setWorkerNode(tempNode);
        } else {
            log.info(String.format("Work %d not assignable. Adding to backlog.", newWork.getWorkID()));
        }
    }
    ...
    private Node assignWorkCreated() {
        ...
        if (workManager.isWorkAvailable()) {
            if (nodeManager.getMostFreeNode() == null) {
                log.debug("Work available but no free node found");
                return null;
            } else {
                ...
                log.info("Sending to node " + availableNode.getNodeID() + ": " + messageToSend);
    ...
    private void workComplete(int workID){
        ...
        if (workerNode != null) {
            ...
            log.info(String.format("Work %d completed by node %d", workID, workerNode.getNodeID()));
        } else {
            log.warn(String.format("Work %d completed but no worker node recorded", workID));
        }
    }
    ...
    public void runSystem() {
        log.info("Running System on port: " + serverPort);
        InetAddress tempNodeIP;
        int tempNodePort;
        int tempMaxJobs;
        try (DatagramSocket socket = new DatagramSocket(serverPort)) {
            socket.setSoTimeout(0);
            log.info(String.format("Socket opened on %s:%d", InetAddress.getLocalHost().getHostAddress(), serverPort));
            log.debug("Entering main receive loop");
            ...
            while (systemOnline) {
                ...
                log.debug(String.format("Packet received from %s:%d (len=%d)", packet.getAddress(), packet.getPort(), packet.getLength()));
                String messages = new String(buffer);
                String[] elements = messages.trim().split(",");
                if (elements.length == 0 || elements[0].trim().isEmpty()) {
                    log.warn("Received empty or malformed message from " + packet.getAddress() + ":" + packet.getPort() + " -> raw='" + messages.trim() + "'");
                    continue;
                }
                String command = elements[0].trim();
                // log received message with source and parse summary
                log.debug(String.format("Parsed command='%s' parts=%d raw='%s'", command, elements.length, messages.trim()));
                log.info(String.format("Received '%s' from %s:%d", messages.trim(), packet.getAddress(), packet.getPort()));
                switch (command) {
                    case "NEWWORK":
                        try {
                            ...
                            log.warn("Invalid NEWWORK duration: " + (elements.length > 1 ? elements[1] : "<missing>") + " raw='" + messages.trim() + "'", exception);
                        }
                        break;
                    case "SHUTDOWN":
                        shutdown();
                    ...
                    case "NEW":
                        try {
                            ...
                            log.info(String.format("New node accepted: %s:%d (id=%d, maxJobs=%d)",
                                    tempNodeIP.getHostAddress(), tempNodePort, currentNode.getNodeID(), tempMaxJobs));
                        } catch (UnknownHostException | NumberFormatException exception) {
                            log.warn("Invalid NEW node parameters raw='" + messages.trim() + "'", exception);
                        }
                        break;
                    case "READY":
                        log.debug("READY received — attempting to assign work");
                        assignWorkCreated();
                        break;
                    case "COMPLETE":
                        try {
                            ...
                            log.warn( "Invalid COMPLETE work id: " + (elements.length > 1 ? elements[1] : "<missing>") + " raw='" + messages.trim() + "'", exception);
                        }
                        assignWorkCreated();
                        break;
                    case "FAILEDWORK":
                        try {
                            ...
                            log.warn("Removing bad node from operation: " + (badNode != null ? badNode.getNodeID() : "unknown"));
                            if (badNode != null) {
                                badNode.sendMessageToNode("SHUTDOWN");
                                nodeManager.delete(badNode);
                                log.debug("Sent SHUTDOWN to bad node id=" + badNode.getNodeID());
                            }
                            createNewWork(tempWork.getDuration());
                            tempWork.setWorkerNode(null);
                            workManager.workComplete(tempWork);
                        } catch (NumberFormatException exception) {
                            log.warn( "Invalid FAILEDWORK work id: " + (elements.length > 1 ? elements[1] : "<missing>") + " raw='" + messages.trim() + "'", exception);
                        }
                        break;
                    case "ALIVE":
                    case "WORKING":
                        try {
                            ...
                            if (currentNode != null) {
                                ...
                                log.debug(String.format("Node check-in: %s:%d (id=%d)", tempNodeIP.getHostAddress(), tempNodePort, currentNode.getNodeID()));
                            } else {
                                log.warn(String.format("Check-in from unknown node %s:%d", tempNodeIP.getHostAddress(), tempNodePort));
                            }
                        } catch (NumberFormatException | UnknownHostException exception) {
                            log.warn( "Invalid ALIVE/WORKING parameters raw='" + messages.trim() + "'", exception);
                        }
                        break;
                    case "DEADNODE":
                        try {
                            ...
                            if (currentNode != null) {
                                ...
                                log.warn("Node deleted due to unresponsiveness: " + currentNode.getNodeID());
                            } else {
                                log.warn(String.format("DEADNODE for unknown node %s:%d", tempNodeIP.getHostAddress(), tempNodePort));
                            }
                        } catch (NumberFormatException | UnknownHostException exception) {
                            log.warn( "Invalid DEADNODE parameters raw='" + messages.trim() + "'", exception);
                        }
                        break;
                    default:
                        log.warn("I don't understand: {} raw='{}'", elements[0], messages.trim());
                }
            }
        } catch (IOException exception) {
            log.error("I/O error in runSystem", exception);
        }
    }
}
